...
/// <inheritdoc/>
internal class ApexChartServiceMaui : IApexChartService
{
	/// <summary>
	/// Initialize a new instance of the <see cref="ApexChartServiceMaui"/> class
	/// </summary>
	/// <param name="jSRuntime"></param>
	/// <param name="serviceOptions"></param>
	public ApexChartServiceMaui(IJSRuntime jSRuntime, ApexChartsServiceOptions serviceOptions)
	{
	...
	{
		localeResource.Locale ??= await ReadLocale(localeResource.GetFileName());

		if (localeResource.Locale != null)
	...
	/// <summary>
	/// Reads and deserializes the locale for the specified file.
	/// </summary>
	/// <param name="path"></param>
	private async Task<ChartLocale?> ReadLocale(string path)
	{
		try
		{
			using var stream = await FileSystem.Current.OpenAppPackageFileAsync($"wwwroot/_content/Blazor-ApexCharts/{path}");

			var reader = new StreamReader(stream);
			var text = await reader.ReadToEndAsync();

			return await JsonSerializer.DeserializeAsync<ChartLocale?>(stream, ChartSerializer.GetOptions());
		}
		catch
		{
			return null;
